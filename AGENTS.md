...
**Board:** [Mod Adjuster Mods board](https://github.com/orgs/gitpush-mod/projects/10)
...
- Issues labelled `bug` or `suggestion` → `🐛-mod-feedback` (auto) **if this bundle is in the tag map**. Check `bot/src/config.js` — Mod Adjuster bundle isn't currently mapped, so nothing mirrors from here yet.
- GitHub releases → persistent thread in `📦-mod-updates` **once mapped**. **Public.**
